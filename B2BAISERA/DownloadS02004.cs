...
namespace B2BAISERA
{
    public partial class DownloadS02004 : Form
    {
        private static string fileType = "S02004";
        private bool acknowledge;
        private string ticketNo = "";
        private string message = "";

        public DownloadS02004()
        {
            InitializeComponent();
        }

        private void DownloadS02004_Load(object sender, EventArgs e)
        {
            ...
            //logEvent.WriteDBLog("", "DownloadS02004_Load", false, "", "Download Finish.", fileType, "SERA");
            //Process.Start("taskkill.exe", "/f /im B2BAISERA_S02004.exe");
            #endregion
            //add try catch by fhi 03.06.2014 
            try
            {
                LoginAuthentication();
                if (acknowledge == false || ticketNo == string.Empty)
            ...
            {
                //log file error
                logEvent.WriteDBLog("", "DownloadS02004_Load", false, "", ex.Message, fileType, "SERA");
                Process.Start("taskkill.exe", "/f /im B2BAISERA_S02004.exe");
            }
        }
        ...
        private void Download()
        {
            ...
            try
            {
                using (wsB2B.B2BAIWebServiceDMZ wsB2B = new wsB2B.B2BAIWebServiceDMZ())
                {
                    ...
                    logEvent.WriteDBLog("B2BAIWebServiceDMZ", "DownloadDocumentS02004", acknowledge, ticketNo, message, fileType, "SERA");
                    LblResult.Text = "Service Result = ";
                    LblAcknowledge.Text = "Acknowledge : " + acknowledge;
                    LblTicketNo.Text = "TicketNo : " + ticketNo;
                    LblMessage.Text = "Message :" + message;
                    ...
                    if (listTransactionDataModel.Count > 0)
                    {
                        ...
                        updateTransaction = transactionProvider.UpdateTransactionS02004(listTransactionDataModel); //1;

                        //3. SUCCEDED OR FAILED, CALL METHOD UPDATEDOCUMENTSTATUS
                        if (updateTransaction == 1)
                        {
                            //4. INSERT INTO CUSTOM_TRANSACTION + CUSTOM_TRANSACTIONDATA + CUSTOM_TRANSACTIONDATADETAIL + CUSTOM_S02004
                            insertTransaction = transactionProvider.InsertLogTransactionDownloadS02004(acknowledge, ticketNo, message, "Succeeded", listTransactionDataModel);
                        ...
                        else
                        {
                            ...
                            //4. INSERT INTO CUSTOM_TRANSACTION + CUSTOM_TRANSACTIONDATA + CUSTOM_TRANSACTIONDATADETAIL + CUSTOM_S02004
                            insertTransaction = transactionProvider.InsertLogTransactionDownloadS02004(acknowledge, ticketNo, message, "Failed", listTransactionDataModel);
                            if (insertTransaction == 1)
                            {
                        ...
                        logEvent.WriteDBLog("B2BAIWebServiceDMZ", "UpdateDocumentStatusS02004", acknowledge, ticketNo, message, fileType, "SERA");
        ...
        private wsB2B.DownloadRequest SampleDataDownloadRequestNew()
        {
            ...
            downloadRequest.FileType = "S02004";
            downloadRequest.SourceUser = "AI";
            downloadRequest.Status = "New";
            downloadRequest.TransDateFrom = DateTime.Now.AddDays(-7);//Convert.ToDateTime("2013-10-29T00:00:00", new CultureInfo("id-ID"));
            downloadRequest.TransDateTo = DateTime.Now;//Convert.ToDateTime("2013-10-30T00:00:00", new CultureInfo("id-ID"));
            downloadRequest.Key1 = ""; //tebet sap"T212"; //garuda pss"T153";
            downloadRequest.Key2 = "";
            downloadRequest.Key3 = ""; //"4111021714";
